...
function collectPiece()
    ...
    if noOfPiecesCollected >= noOfPieces then
      composer.removeScene("scenes.jumping4", false)
      composer.gotoScene("scenes.win4", {time = 1000, effect = "slideUp"})
  end
end
...
function moveBarrel2(index)
    if(index == 1) then
        barrel.x = math.max(570, barrel1.x + 200) + math.random (-10,10)--Respawning of the barrel
    elseif (index == 2) then
        barrel1.x = math.max(570, barrel.x + 150) + math.random (-10,10) -- Respawning of Barrel 2
    end
end
